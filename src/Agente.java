 ...
 * Assume-se que:
 * 
 * - O agente nunca morre, pois ele sempre desvia o wumpus e os buracos;
 * - O agente ganha o jogo quando coleta o ouro se o mesmo estiver ali;
 * - O agente para quando termina o jogo ou quando todos os objetivos no mundo foram completos.
 * 
 * @author levymateus
 *
 */
public class Agente {
	...
	public void buscaLargura(Grafo g, Grafo.Vertice vIni) {
		...
		for(int i = 0; i < g.vertices; i++) visitado[i] = false;
		
		visitado[vIni.indice] = true;
		vProx = vIni; // o proximo passo ̩ o vertice inicial
		historico.add(vProx);
		...
		while(pontos > -1000) {
			...
			/**
			 * O agente olha para todos os vertices adjacentes
			 * e escolhe qual vertice adjacente sera o proximo passo
			 * com base no perigo de cada vertice adjacente.
			...
			while(it.hasNext()) {
				...
				if(!visitado[ vFoco.indice ]) {
					...
					/**
					 * Se o vertice que o agente estiver olhando for o wumpus VIVO, entao atira uma flecha
					 * se tiver flecha entao o wumpus morre, caso contrario o agente tera que realizar outra acao.
					 */
			...
			historico.add(vProx);// O agente armazena em sua memoria o caminho feito adicionando cada passo em uma pilha.
			System.out.println("\n");
			perigo = -9000;
		}
